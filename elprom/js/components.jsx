/* ELPROM — Shared component library (exported to window) */
...
const ICONS = {
  arrowRight: <><line x1="4" y1="12" x2="19" y2="12" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/>{P("M13 6l6 6-6 6")}</>,
  sparkle: P("M12 3l1.6 4.7L18 9l-4.4 1.3L12 15l-1.6-4.7L6 9l4.4-1.3z"),
  wand: <>{P("M5 19l9-9")}{P("M14 5l1 2 2 1-2 1-1 2-1-2-2-1 2-1z")}</>,
  copy: <>{P("M9 9h9v9a2 2 0 0 1-2 2H9a2 2 0 0 1-2-2V9z")}{P("M5 15H4a1 1 0 0 1-1-1V5a2 2 0 0 1 2-2h8a1 1 0 0 1 1 1v1")}</>,
  download: <>{P("M12 4v10")}{P("M8 11l4 4 4-4")}{P("M5 19h14")}</>,
  refresh: <>{P("M4 12a8 8 0 0 1 13.7-5.6L20 8")}{P("M20 4v4h-4")}{P("M20 12a8 8 0 0 1-13.7 5.6L4 16")}{P("M4 20v-4h4")}</>,
  share: <>{P("M14 9l6-5v16l-6-5H6a2 2 0 0 1-2-2v-2a2 2 0 0 1 2-2z")}</>,
  chevronDown: P("M6 9l6 6 6-6"),
  chevronRight: P("M9 6l6 6-6 6"),
  check: P("M5 12l5 5 9-11"),
  lock: <>{P("M6 11h12v8H6z")}{P("M9 11V8a3 3 0 0 1 6 0v3")}</>,
  sun: <>{P("M12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8z")}{P("M12 2v2M12 20v2M4 12H2M22 12h-2M5 5l1.5 1.5M17.5 17.5L19 19M19 5l-1.5 1.5M6.5 17.5L5 19")}</>,
  moon: P("M20 14.5A8 8 0 0 1 9.5 4a7 7 0 1 0 10.5 10.5z"),
  plus: P("M12 5v14M5 12h14"),
  x: P("M6 6l12 12M18 6L6 18"),
  calendar: <>{P("M5 7h14v13H5z")}{P("M5 11h14")}{P("M9 4v3M15 4v3")}</>,
  ...
  settings: <>{P("M12 9a3 3 0 1 0 0 6 3 3 0 0 0 0-6z")}{P("M19 12a7 7 0 0 0-.1-1.2l2-1.6-2-3.4-2.3 1a7 7 0 0 0-2-1.2l-.4-2.5H9.8l-.4 2.5a7 7 0 0 0-2 1.2l-2.3-1-2 3.4 2 1.6A7 7 0 0 0 5 12a7 7 0 0 0 .1 1.2l-2 1.6 2 3.4 2.3-1a7 7 0 0 0 2 1.2l.4 2.5h4.4l.4-2.5a7 7 0 0 0 2-1.2l2.3 1 2-3.4-2-1.6A7 7 0 0 0 19 12z")}</>,
  grid: <>{P("M4 4h7v7H4zM13 4h7v7h-7zM4 13h7v7H4zM13 13h7v7h-7z")}</>,
  trend: <>{P("M4 16l5-5 4 4 7-8")}{P("M16 7h5v5")}</>,
  globe: <>{P("M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18z")}{P("M3 12h18")}{P("M12 3c2.5 2.4 4 5.6 4 9s-1.5 6.6-4 9c-2.5-2.4-4-5.6-4-9s1.5-6.6 4-9z")}</>,
  shield: <>{P("M12 3l7 3v6c0 4-3 7-7 9-4-2-7-5-7-9V6z")}{P("M9 12l2 2 4-4")}</>,
  edit: <>{P("M4 20h4l10-10-4-4L4 16z")}{P("M14 6l4 4")}</>,
  ...
  google: <g>
    ...
    <path fill="#FBBC05" d="M6.4 14c-.2-.6-.3-1.3-.3-2s.1-1.4.3-2V7.4H3.1A10 10 0 0 0 2 12c0 1.6.4 3.2 1.1 4.6z"/>
    <path fill="#EA4335" d="M12 5.9c1.5 0 2.8.5 3.8 1.5l2.8-2.8A10 10 0 0 0 3.1 7.4l3.3 2.6C7.2 7.6 9.4 5.9 12 5.9z"/>
  </g>,
};
...
function Button({ variant='primary', size='md', loading, children, onClick, className='', icon, iconRight, style, disabled, title, 'aria-label':ariaLabel }) {
  ...
  return (
    <button ref={ref} className={`btn btn-${variant} ${sizeC} ${className}`} onClick={handle} disabled={disabled} style={style} title={title} aria-label={ariaLabel}>
...
function ThemeToggle({ theme, onToggle }) {
  return (
    <button className="theme-toggle" onClick={onToggle} aria-label="Toggle dark mode" role="switch" aria-checked={theme==='dark'}>
      <span className="knob"><Icon name={theme==='dark'?'moon':'sun'} size={14} /></span>
    </button>
  );
}
...
function Chip({ active, children, onClick, icon, className='' }) {
  return (
    <button className={`chip ${active?'chip-active':''} ${className}`} onClick={onClick}>
...
function FormatChip({ type }) {
  ...
  return <span className={`format-chip ${map[type]}`}><Icon name={icon} size={11} />{type}</span>;
}
...
function Dropdown({ trigger, children, align='right' }) {
  ...
  return (
    <div className="dropdown" ref={ref}>
      ...
      {open && (
        <div className="dropdown-menu" style={{ top:'calc(100% + 8px)', [align]:0 }} onClick={()=>setOpen(false)}>
          {children}
        </div>
      )}
    </div>
  ...
  return (
    <ToastCtx.Provider value={push}>
      ...
      <div className="toast-wrap">
        {toasts.map(t => <div className="toast" key={t.id}><span className="toast-dot" />{t.msg}</div>)}
      </div>
    </ToastCtx.Provider>
  );
}
...
function Meter({ value, max=1 }) {
  return <div className="meter"><div className="meter-fill" style={{ width:`${Math.min(100,(value/max)*100)}%` }} /></div>;
}
...
function Modal({ onClose, children, style, className='', sheetOnMobile }) {
  ...
  return (
    <div className="scrim" onClick={onClose}>
      <div className={`modal ${className}`} style={style} onClick={e=>e.stopPropagation()} role="dialog" aria-modal="true">
        {children}
      </div>
    </div>
  );
...
Object.assign(window, {
  Icon, Button, ThemeToggle, Chip, FormatChip, Dropdown,
  ToastProvider, useToast, Sk, Meter, Modal, Rise, useRise,
});
